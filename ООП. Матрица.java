...
class Matrix {
    ...
    public static Matrix multiply(Matrix firstMatrix, Matrix secondMatrix) {
        if (firstMatrix.m != secondMatrix.n)
            throw new IllegalArgumentException("Чтобы умножить матрицу нужно, чтобы число столбцов матрицы равнялось числу строк матрицы.");
    ...
    public Matrix sum(Matrix secondMatrix) {
        if (!isSizeSame(secondMatrix))
            throw new ArithmeticException("Складывать можно только матрицы одинакового размера.");
...
class RectangleMatrix extends Matrix {
    RectangleMatrix(int m, int n) {
        super(m, n);
        if (m == n)
            throw new ArithmeticException("Прямоугольная матрица — это матрица, у которой число строк не равно числу столбцов.");
    }
    ...
    public static void getCof(double matrix[][], double temp[][], int p, int q, int n) {
        int i = 0, j = 0;
